...
if (isset($_POST['btnSave'])){
	...
	$errMsg .= checkUpload($_FILES["file"],".jpg;.gif;.bmp;.pdf;.jpeg",5000*5000,0);

	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update tbl_quanhuyen_category set name='".$name."' , sort='".$sort."' ,typebando='".$typebando."' , last_modified=now()  where id='".$oldid."'";
		}else{
		...
		if (mysql_query($sql,$conn)){
			...
			if ($_FILES['file']['name']!=''){
				$extlarge=getFileExtention($_FILES['file']['name']);
				if($extlarge=="pdf"){
					$typebando="pdf";
				}
				$path_up_file="$path/bando_files/$oldid$extlarge";
				if (makeUpload($_FILES['file'],$path_up_file)){
					@chmod("$path/bando_files/$oldid$extlarge", 0777);
					if($sqlUpdateField != "") $sqlUpdateField .= ",";
					$sqlUpdateField .= " file='images/bando_files/$oldid$extlarge' ";
				}
			}
                              ...
                              <tr>
								...
								<tr>
                                  <td valign="top" width="30%">
                                      Bản đồ</td>
                                  <td valign="middle" width="70%">
                                      <input type="file" name="file"> (png,jpg,jpeg,bmp,pdf)
									  <?php echo $file_bd!=""?"<a href='$linkroot/$file_bd' target='_blank'>Xem file</a>":"";?>
                                  </td>
                                </tr>
                                <tr>
                                    <td valign="top" width="30%">&nbsp;
