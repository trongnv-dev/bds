<? /*********************************************************************************************************/
/***************************************** Check PHP Version *********************************************/ /*echo 'Current PHP version: ' . phpversion(); /*if (phpversion()< "4.1.0") { $_GET = $HTTP_GET_VARS; $_POST = $HTTP_POST_VARS; $_SERVER = $HTTP_SERVER_VARS; } /*********************************************************************************************************/
...
function createPage($total, $link, $nitem, $itemcurrent, $step = 10)
{
    ...
    for ($i = $from; $i < $to; $i++) {
        if ($i != $itemcurrent) $ret .= '<a href="' . $link . $i . '" class="lslink">' . ($i + 1) . '</a> '; else $ret .= '<b>' . ($i + 1) . '</b> ';
    }
...
{
    $direction = "";
    if ($_REQUEST['direction'] == '' || $_REQUEST['direction'] != '1') $direction = "1"; else $direction = "0";
...
function checkUpload($f, $ext = "", $maxsize = 0, $req = 0)
{
    ...
    if ($fsize == 0) {
        ...
        if ($ext != "") if (strpos($ext, $fext) === false) return "T&#7853;p tin kh&ocirc;ng &#273;&uacute;ng &#273;&#7883;nh d&#7841;ng : $fname";
        if ($maxsize > 0) if ($fsize > $maxsize) return "K&iacute;ch th&#432;&#7899;c h&igrave;nh ph&#7843;i nh&#7887; h&#417;n " . $maxsize . " byte";
    }
    return "";
}
...
function countRecord($table, $where = "")
{
    ...
    if ($table == "") return false;
    if ($where == "") $where = "1=1";
    $_sql = "select count(*) as cnt from $table where $where";
    $result = mysql_query($_sql, $conn);
...
function getParent($table, $id)
{
    ...
    while ($dem > 0) {
        ...
        if ($dem != 0) {
            ...
            $ddk = substr($ddk, 1);
            $parent = $ddk;
            $mang .= $parent . ","; /* $mang=substr($mang,1);*/
        }
    }
    $mang .= $id;
    return $mang;
...
function getArrayCategory($table, $catid = "", $split = "=")
{
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        foreach ($getsub as $sub) $ret[] = array($sub[0], $sub[1]);
    }
    return $ret;
}
...
function comboLanguage($name, $langSelected, $class)
{
    ...
    foreach ($arrLanguage as $lang) {
        if ($lang[0] == $langSelected) $out .= '<option value="' . $lang[0] . '" selected>' . $lang[1] . '</option>'; else $out .= '<option value="' . $lang[0] . '">' . $lang[1] . '</option>';
    }
    $out .= '</select>';
    return $out;
}
...
function comboCategory1($name, $table, $arrSource, $class, $index, $all, $shop)
{
    ...
    foreach ($cats as $cat) {
        ...
        if ($result > 0) $out .= '<option value="' . $cat[0] . '" ' . $selected . '>' . $cat[1] . '</option>';
    }
    $out .= '</select>';
    return $out;
}
...
{
    $arrValue = array('0', '1', '2', '3', '4', '5', '6');
    $arrTextVN = array('1 tuần', '1 tháng', '2 tháng', '3 tháng', '6 tháng', '9 tháng', '1 năm');
    $arrTextEN = array();
    $arrText = $lang == "vn" ? $arrTextVN : $arrTextEN;
...
{
    $arrValue = array('0', '1', '2', '3', '4', '5');
    $arrTextVN = array('Sổ đỏ', 'Sổ hồng', 'Hợp đồng dự án', 'Công chứng ủy quyền', 'Giấy tay', 'Khác');
    $arrTextEN = array();
    $arrText = $lang == "vn" ? $arrTextVN : $arrTextEN;
    ...
    if ($kieux == 1) {
        ...
        for ($i = 0; $i < count($listid); $i++) {
            ...
            if ($hinh == 1) {
                ...
                $sql = "DELETE FROM $table $id";
                if ($idx > 0) mysql_query($sql, $conn) or die(mysql_error());
            } elseif ($hinh == 0) {
                $sql = "DELETE FROM $table $id";
                if ($idx > 0) mysql_query($sql, $conn) or die(mysql_error());
            }
...
function mail_smtp($from, $to, $subject, $body, $html = 0)
{
    ...
    $smtp = new smtp_class;
    $smtp->host_name = "112.78.4.202";       /* Change this variable to the address of the SMTP server to relay, like "smtp.myisp.com" */
    ...
    $smtp->pop3_auth_host = "mail.introvina.com";           /* Set to the POP3 authentication host if your SMTP server requires prior POP3 authentication */
    $smtp->user = "vcthth@gmail.com";                     /* Set to the user name if the server requires authetication */
    $smtp->realm = "";                    /* Set to the authetication realm, usually the authentication user e-mail domain */
    $smtp->password = "introvina.com";                 /* Set to the authetication password */
    ...
    $header = "";
    if ($html == 0) $header = array("From: $from", "To: $to", "Subject: $subject", "Date: " . strftime("%a, %d %b %Y %H:%M:%S %Z")); else $header = array("MIME-Version: 1.0", "Content-type: text/html; charset=utf-8", /*"Content-type: text/html; charset=iso-8859-1",*/
        "From: $from", "To: $to", "Subject: $subject", "Date: " . strftime("%a, %d %b %Y %H:%M:%S %Z"));
...
function check_table($table, $where, $id)
{
    ...
    if ($row_rs[0] >= 1) return false; else return true;
}
...
function pagesLinks($totalRows, $pageSize = 5)
{
    ...
    if (isset($_GET["pageNum"]) == true) $currentPage = $_GET["pageNum"]; else $currentPage = 1;
...
function pagesListLimit_new_2013($totalRows, $pageSize = 5, $offset = 5, $host_link_full, $tukhoa, $get_p)
{
    if ($totalRows <= 0) return "";
    $totalPages = ceil($totalRows / $pageSize);
    if ($totalPages <= 1) return "";
...
function pagesLinks_new_full_2013($totalRows, $pageSize = 5, $host_link_full, $tukhoa, $get_p)
{
    if ($totalRows <= 0) return "";
    $totalPages = ceil($totalRows / $pageSize);
    if ($totalPages <= 1) return "";
...
function pagesLinks_new_full_2014($totalRows, $pageSize = 5, $host_link_full, $get_p)
{
    if ($totalRows <= 0) return "";
    $totalPages = ceil($totalRows / $pageSize);
    if ($totalPages <= 1) return "";
...
function pagesListLimit_new_2014($totalRows, $pageSize = 5, $offset = 5, $host_link_full, $get_p)
{
    if ($totalRows <= 0) return "";
    $totalPages = ceil($totalRows / $pageSize);
    if ($totalPages <= 1) return "";
...
function luu_hinh($filechon, $dthem, $uploaddir, &$error)
{
    ...
    $error = "";
    $choupload = array("image/gif", "image/jpeg", "image/pjpeg", "image/png", 'image/x-png', 'image/x-icon');
    $maxsize = 1024 * 5000;
    $f = $_FILES[$filechon];
    $tmp_name = $f["tmp_name"];
    if ($tmp_name == "") return "";
    ...
    if ($kieufile == "image/png" || $kieufile == "image/x-png") $ext = ".png"; elseif ($kieufile == "image/gif") $ext = ".gif";
    elseif ($kieufile == "image/x-icon") $ext = ".ico";
    else $ext = ".jpg";
...
function cat_chuoi_lam_tags($noidung, $check, $num, $links_goc, $target = 0)
{
    ...
    if ($check == 1 || $a == 1) {
        foreach ($cut as $ch) {
            if ($ch != "") {
                $kq = $kq . '<a ' . $targets . ' href="' . $links_goc . convert_character_db_search($ch, 1) . '.html">' . $ch . '</a>  ';
            }
        }
    } else {
        foreach ($cut as $ch) {
...
function userPermiss($iduser, $cate, $link_nhay)
{
    settype($iduser, "int");
    settype($cate, "int");
    $user = getRecord('tbl_users', "id='" . $iduser . "'");
    $idgroup = $user['idgroup'];

    if ($idgroup == 3) { //mod va nhap lieu
        $listCity = $user['listCity'];// kiem tra tinh
        $testCity = number_in_list($listCity, $cate);
        if ($testCity == 0) {
            $_SESSION['error'] = "Bạn không có quyền vào tỉnh này..!";
            header("location: $link_nhay");
        }
...
function userPermissEdit($listEdit, $cate, $type)
{
    $tach = explode("m", $listEdit); // chuyen muc

    foreach ($tach as &$value) {
        $tach2 = explode("o", $value);

        if ($tach2[0] == $cate) {
            $chuoi = substr($tach2[1], 1);

            $tach3 = explode(",", $chuoi);

            foreach ($tach3 as &$value1) {
                if ($type == $value1) {
                    return true;
                    break;
                }
