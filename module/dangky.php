...
if (isset($_POST['btn_dangky'])==true)//isset kiem tra submit
	{
		...
		{  
			...
			$khoa=1;
			$kichhoatx=1;
			$vale1='username,password,name,cmnd,mobile,email,date_added,active,idgroup,status,randomkey';
			$vale2="'".$tendk."','".$password."','".$hoten."','".$cmnd."','".$dienthoai."','".$email."','"
...
$(document).ready(function() {	
	...
	$("form[id=form1]").bind('submit',function(){
		...
		if(dienthoai=="") {
			alert("Bạn chưa nhập số điện thoại đăng ký");
			return false;
		}
		if(user_s <6 ){
             ...
             <form id="form1" name="form1" method="post" action="<?php echo $linkrootbds;?>dang-ky.html">
                <ul>
                    ...
                    <li>
					 ...
					 <li>
                        <span class="l_k_dn2">
                            Số điện thoại <span class="color_btd">*</span>
                        </span><!-- End .l_k_dn2 -->
                        <span class="r_k_dn2">
                            <input class="box-sizing-fix ipt_dn2"  required   type="text" id="dienthoai" name="dienthoai"  value="<?php echo $dienthoai; ?>" placeholder="Bạn vui lòng nhập số điện thoại..!" >
                        </span><!-- End .r_k_dn2 -->
                        <?php if($coloi_hien_cmnd!=""){?> 
                         <span class="erros_fu_dndk" id="dienthoai"><?=$dienthoai?></span>
                         <? }?>
                    </li>
					<?php /*
                    <li class="mbm2">
                ...
                <span class="erros_fu_dndk" <?php if($coloi!=true){?> style="display: none;" <?php }?>> <?=$coloi_hien_tendk.$coloi_hien_matkhau.$coloi_hien_golaimatkhau.$coloi_hien_hoten.$coloi_hien_email.$coloi_hien_cap.$coloi_hien_dienthoai;?></span>
             </form>
            </div><!-- End .m_khung_dn2 -->
