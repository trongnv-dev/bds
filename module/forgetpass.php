	...
	if (isset($_POST['btn_doipass'])==true) {
		...
		if ($rowtin['id'] == ""){
			   ...
			   echo "<script language='javascript'>alert('Email này sai hoặc chưa tồn tại trong hệ thống, vui lòng kiểm tra lại..!');</script>";
				...
				$tieude="Phục hồi mật khẩu - BDSTPHCM.ORG";
				...
				if($email!=""){	
					...
					<br> Nhấp vào link này để tiến hành xác nhận phục hồi mật khẩu <a href='".$host_link_full."/index.php?act=getpass&randomkey=".$randomkey."'> click here </a>";
					$noidung=$noidung_AltBody;
