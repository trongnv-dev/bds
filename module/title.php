<?php
	...
	if($_GET['act']=="linhtinh"){ // tim kiem
		//http://www.bdstphcm.com/ho-chi-minh/quan-1/phuong-ben-nghe/800-trieu-den-1-ti/
		$binhthuong=1;
		$field=array();
		$dem_para=0;
		for($i=1;$i<9;$i++){
...
<link rel="canonical" href="http://bdstphcm.com<?=$_SERVER['REQUEST_URI']?>" />
<?php if($dem_para<8){?>
<?php 
if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
if ($pageNum<=0) $pageNum=1;
...
<meta property='og:url' content='http://bdstphcm.com<?=$_SERVER['REQUEST_URI']?>'/>
...
<?
	...
	elseif($_GET['act']=='product_detail' && $_GET['bds']){ // chi tiet tin - danh muc bds,da,dn - list bds theo thanh pho
		...
		if($row_rs_tit['id']!=""){// chi tiet tin
				//http://www.bdstphcm.com/athena-building-van-phong-cho-thue-quan-tan-binh.html
...
<?				
		...
		if($row_rs_tit['id']==""){// danh muc
				$binhthuong=1;
				//http://www.bdstphcm.com/cho-thue-dat.html
				//http://www.bdstphcm.com/tin-tuc-bat-dong-san.html
...
<?				}			
	    ...
	    if($row_rs_tit['id']=="" && $a['id']>0) { // Thành phố
			$binhthuong=1;
			//http://www.bdstphcm.com/da-nang.html 
...
<?				
	...
	if($dem_ct==1 && $tpvaduong==0){
		...
		if($duong['id']>0 ){
			$binhthuong=1;
			// Thành Phố / Đường
			//http://www.bdstphcm.com/da-nang-duong-dang-thuy-tram.html
...
<?				
	...
	// Thành phố / Huyện
	//http://www.bdstphcm.com/binh-duong/thi-xa-di-an.html
...
<link rel="canonical" href="http://www.bdstphcm.com<?=$_SERVER['REQUEST_URI']?>" />
<?php 
if (isset($_GET['pageNum'])==true) $pageNum = $_GET['pageNum'];
if ($pageNum<=0) $pageNum=1;
if($pageNum>1){
...
<?				
	...
	//www.bdstphcm.com/ho-chi-minh/huyen-cu-chi/xa-phuoc-vinh-an.html
...
//Nhu Cầu
	elseif($_GET['act']=='nhucau'){
		$binhthuong=1;
		//http://www.bdstphcm.com/binh-duong/thi-xa-di-an/phuong-dong-hoa/nha-dat-ban.html
		$cate=$_GET['nhucau'];
